...
if(isset($_POST["name"])){
    try {
        ...
        try {
            ...
            // Send separate email to multiple admins
            $adminEmails = ['meenaweb1997@gmail.com', 'meenatchipkr@gmail.com']; // Add more admin emails as needed
            foreach ($adminEmails as $adminEmail) {
                $mail->ClearAllRecipients();
                $mail->addAddress($adminEmail); // Add admin email
            ...
            echo json_encode(['result' => 'error', 'error' => "Message could not be sent. Mailer Error: {$mail->ErrorInfo}"]);
        }
    } catch (PDOException $e) {
        echo json_encode(['result' => 'error', 'error' => 'Error: ' . $e->getMessage()]);
    }
} else {
    echo json_encode(['result' => 'error', 'error' => 'Error: No data received']);
}
?>
